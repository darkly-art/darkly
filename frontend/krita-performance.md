...
| Flag | Behavior |
|------|----------|
| `NORMAL` | Takes a read-lock, so multiple normal jobs run concurrently |
| `EXCLUSIVE` | Takes a write-lock, blocking all other jobs |
...
too, since the same patterns apply directly to tile-based compositing on the GPU.
