...
// to Vite, which watches only `frontend/`. So every edit under `crates/` —
// a shader, a veil's `preview_at`, a registration — needed a manual
// `npm run wasm:build-dev` and a manual reload before it showed up in the
// editor, and a dev server left running silently served stale WASM. This
// closes that: the same watch-and-regenerate shape `iconBundlePlugin` uses for
...
/// Run the dev bridge build, resolving `{ ok, output }`. Never rejects — a
/// failed build is a normal state during editing, and the watcher has to
/// survive it to pick up the fix.
///
