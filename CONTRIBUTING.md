...
| Making a release, the version string, or anything a `v*` tag triggers | [`docs/versioning.md`](docs/versioning.md) | "Cutting a release" there is the whole procedure, every step from choosing the version to a published release, including the release PR, whose body is a plain list of PR links rather than the two-part form. The annotated tag is the release record; the metainfo's release history is generated from the tags and committed after each release. The version itself is derived from git tags at build time and baked twice (Rust and Vite) as declared canonical twins; the `Cargo.toml` / `package.json` values are vestigial |
