...
//! that and still read as a test card rather than as a picture, which is the
//! one thing a preview cannot afford: a reader decides whether an effect is
//! worth trying by looking at 72 px of it in a table.
//!
...
/// nothing.** [`resample`] serves a request above the stored size by exact
/// pixel replication, and a 2:1 area average of a pixel-replicated image is the
/// image again, so the target sees these very pixels either way. The only
/// difference a 512 px file would make is which resampler downsized the
/// original artwork (this module's box filter instead of the one that produced
/// the file), and the file wins that comparison.
...
/// quadrants, so a `2 · dim` render is an exact supersample of the `dim` one
/// for *any* `dim`, whether it divides the stored size or not.
///
/// Above the stored size the same rule degenerates to exact pixel replication
/// (an output box that falls inside one source pixel integrates to that
/// pixel), which is what makes [`SUBJECT_SRC_DIM`] the render size rather than
/// twice it.
///
...
/// between two non-symmetric mid-tones, held away from the 0 and 1 boundaries
/// so every mode is tested on its interior rather than on an edge case, the
/// same reasoning behind the fixed colour pair in `tests/blend_modes.rs`. Two
/// busy images blended together read as neither.
...
mod tests {
    ...
    /// target's resample is a clean 2:1 area average; averaging a replicated
    /// block returns the pixel it came from, so that target sees the stored
    /// image exactly, unsoftened, without a 512 px file existing. If this ever
    /// stops holding, the doubled request stops being free and the stored size
    /// has to grow with it.
    #[test]
