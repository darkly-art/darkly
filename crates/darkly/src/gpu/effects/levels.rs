...
//! over the shared LUT scaffold — it hands [`bake_lut`] eight
...
fn create_pipeline(device: &wgpu::Device, format: wgpu::TextureFormat) -> EffectPipeline {
    lut_pipeline(device, format, "levels")
}

fn kind() -> Arc<ParamEffectKind> {
    lut_kind("levels", "levels", PARAMS, build_lut)
}
