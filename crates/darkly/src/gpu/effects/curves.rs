...
//!
//! Curves is a thin provider over the shared LUT scaffold: it
...
use crate::gpu::effect::{EffectPipeline, EffectRegistration, COLOR_TARGETS};
use crate::gpu::lut_filter::{bake_lut, lut_kind, lut_pipeline, Baked};
use crate::gpu::param_effect::ParamEffectKind;
...
fn create_pipeline(device: &wgpu::Device, format: wgpu::TextureFormat) -> EffectPipeline {
    lut_pipeline(device, format, "curves")
}

fn kind() -> Arc<ParamEffectKind> {
    lut_kind("curves", "curves", PARAMS, build_lut)
}
