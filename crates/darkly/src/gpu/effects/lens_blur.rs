...
pub fn register() -> EffectRegistration {
    EffectRegistration {
        ...
        from_params: |params, shared| {
            let (radius, threshold) = read_params(params);
...
fn preview_params(t: f32) -> Vec<ParamValue> {
    vec![ParamValue::Float(swing(t)), ParamValue::Float(0.1)]
}
...
fn read_params(params: &[ParamValue]) -> (f32, f32) {
...
fn create_lens_blur_pipeline(device: &wgpu::Device, format: wgpu::TextureFormat) -> EffectPipeline {
    create_effect_pipeline(
        ...
        include_str!("../../../shaders/effects/lens_blur.wgsl"),
        "fs_lens_blur",
    )
}
