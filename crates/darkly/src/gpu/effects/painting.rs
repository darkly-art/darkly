...
pub fn register() -> EffectRegistration {
    EffectRegistration {
        ...
        from_params: |params, shared| {
            let (kernel_size, sharpness, hardness) = read_params(params);
            Box::new(Painting::new(kernel_size, sharpness, hardness, shared))
        },
    }
}

/// The daubs coarsen and tighten again. Only the kernel size moves; sharpness
/// and hardness hold at their defaults, so what the preview shows is the one
/// control that changes the brush scale.
fn preview_params(t: f32) -> Vec<ParamValue> {
    let mut params: Vec<ParamValue> = PARAMS.iter().map(ParamDef::default_value).collect();
    params[0] = ParamValue::Int((1.0 + 6.0 * swing(t)).round() as i32);
    params
}
...
fn read_params(params: &[ParamValue]) -> (i32, f32, f32) {
    ...
    (kernel_size, sharpness, hardness)
}
...
fn create_painting_pipeline(device: &wgpu::Device, format: wgpu::TextureFormat) -> EffectPipeline {
    create_effect_pipeline(
        ...
        include_str!("../../../shaders/effects/painting.wgsl"),
        "fs_painting",
    )
}
