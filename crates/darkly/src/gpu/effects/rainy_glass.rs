...
pub fn register() -> EffectRegistration {
    EffectRegistration {
        ...
        hotkey_action: "effectRainy_glass",
        ...
        from_params: |params, shared| {
            let (speed, rain_amount, direction, fog_amount, scale) = read_params(params);
...
fn read_params(params: &[ParamValue]) -> (f32, f32, f32, f32, f32) {
    ...
    let rain_amount = params.float_at(1, 0.5);
    let direction = params.float_at(2, 0.0);
    let fog_amount = params.float_at(3, 0.0);
    let scale = params.float_at(4, 1.4);
    (speed, rain_amount, direction, fog_amount, scale)
}
...
fn create_rainy_glass_pipeline(
    ...
    create_effect_pipeline(
        ...
        include_str!("../../../shaders/effects/rainy_glass.wgsl"),
        "fs_rainy_glass",
    )
}
