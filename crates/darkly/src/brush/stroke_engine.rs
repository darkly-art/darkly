...
impl StrokeEngine {
    ...
    /// stroke to stroke.  `dpi` is the owning document's resolution, which
    /// the `document_settings` node reads; a render with no document behind
    /// it passes [`crate::document::DEFAULT_DPI`].
