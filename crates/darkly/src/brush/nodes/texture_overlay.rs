...
/// Texture overlay pipeline.  Reads a dab + a pattern texture, writes
/// the textured dab to a fresh dab texture (REPLACE blend).
pub struct TexOverlayPipeline {
    pipeline: wgpu::RenderPipeline,
    ring: DynamicUniformRing,
    uniform_bind_group: wgpu::BindGroup,
}

impl TexOverlayPipeline {
    fn build(ctx: &BuildContext) -> Self {
        ...
        // group(0) = uniforms, group(1) = dab texture, group(2) = pattern texture.
        let layout = ctx
            .device
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("brush-tex-overlay-layout"),
                bind_group_layouts: &[ctx.uniform_bgl, ctx.dab_bgl, ctx.dab_bgl],
                immediate_size: 0,
            });
        let pipeline = ctx
            .device
        ...
        let (ring, uniform_bind_group) = ctx.make_uniform_ring::<TexOverlayUniforms>(
            "brush-tex-overlay-uniforms",
            "brush-tex-overlay-uniform-bg",
        );
        Self {
            pipeline,
            ring,
    ...
    pub fn write_uniforms(&self, queue: &wgpu::Queue, uniforms: &TexOverlayUniforms) -> u32 {
        self.ring.write(queue, bytemuck::bytes_of(uniforms))
    }
}

impl BrushPipelineEntry for TexOverlayPipeline {
...
fn tex_overlay_pipeline_reg() -> BrushPipelineRegistration {
    BrushPipelineRegistration {
        id: "tex_overlay",
        build: |ctx| Box::new(TexOverlayPipeline::build(ctx)),
    }
}
...
pub fn register() -> BrushNodeRegistration {
    BrushNodeRegistration {
        pipelines: vec![tex_overlay_pipeline_reg()],
...
impl BrushNodeEvaluator for TextureOverlayEvaluator {
    ...
    fn evaluate_gpu(
        ...
        let tex_overlay = gpu.pipelines.get::<TexOverlayPipeline>("tex_overlay");
        let offset = tex_overlay.write_uniforms(gpu.queue, &uniforms);
        ...
        // Render textured dab.
        {
            ...
            pass.set_pipeline(tex_overlay.pipeline());
            pass.set_bind_group(0, tex_overlay.uniform_bind_group(), &[offset]);
