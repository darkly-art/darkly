//! Liquify Compiled terminal — per-dab fragment-pass warp with a
//! per-brush compiled WGSL shader.
//!
//! Same overall outline as [`smudge_compiled`](super::smudge_compiled).
...
//! convention (matching the original dispatch shader's waveshape map:
//! 0 = spike → 1 = square), and the WGSL body inverts the slider
//! value before passing it in. The mapping the user sees:
    ...
    fn build(ctx: &BuildContext, compiled: &CompiledBrush) -> Self {
        ...
        let dabs_bgl = ctx
            ...
            .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("liquify_compiled-dabs-bgl"),
        ...
        let layout = ctx
            ...
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("liquify_compiled-layout"),
                bind_group_layouts: &[
                    ctx.uniform_bgl,
                    &dabs_bgl,
                    ctx.selection_bgl,
...
pub struct LiquifyCompiledPipeline {
    cache: RefCell<HashMap<u64, PerBrushPipeline>>,
}

impl LiquifyCompiledPipeline {
...
impl BrushPipelineEntry for LiquifyCompiledPipeline {
...
fn liquify_compiled_pipeline_reg() -> BrushPipelineRegistration {
    BrushPipelineRegistration {
        id: "liquify_compiled",
        build: |ctx| Box::new(LiquifyCompiledPipeline::build(ctx)),
    }
}
...
pub fn register() -> BrushNodeRegistration {
    BrushNodeRegistration {
        pipelines: vec![liquify_compiled_pipeline_reg()],
        node: NodeRegistration {
            type_id: "liquify_compiled",
            category: "output",
            display_name: "Liquify (Compiled)",
            ports: vec![
...
pub struct LiquifyCompiledEvaluator;

impl LiquifyCompiledEvaluator {
...
impl BrushNodeEvaluator for LiquifyCompiledEvaluator {
    ...
    fn evaluate_gpu(
        ...
        let Some(compiled) = gpu.compiled_brush.clone() else {
            debug_assert!(
                false,
                "liquify_compiled requires compiled_brush on gpu_context"
            );
            return vec![];
        };
        ...
        // Three early-outs — mirror the dispatch-path liquify gates so
        // the queue size and pass count are identical.
        ...
        // `canvas_pos - direction × displacement × falloff(d)` always
        ...
        let outputs = gpu
            .slot_outputs_owned
            .as_ref()
            .expect("liquify_compiled requires slot_outputs_owned on gpu_context");
        ...
        debug_assert!(
            ...
            "liquify_compiled dab queue overflowed MAX_DABS_PER_PHASE"
        );
    ...
    fn flush_dabs(&self, _ctx: &EvalContext, gpu: &mut BrushGpuContext) {
        ...
        let Some(compiled) = gpu.compiled_brush.clone() else {
            debug_assert!(
                false,
                "liquify_compiled::flush_dabs requires compiled_brush"
            );
            return;
        };
        ...
        debug_assert_eq!(
            ...
            "liquify_compiled meta queue out of sync with dab queue"
        );
        ...
        let pipeline_ref = gpu
            .pipelines
            .get::<LiquifyCompiledPipeline>("liquify_compiled");
        ensure_per_brush_pipeline(gpu, pipeline_ref, &compiled);

        let paint_target = gpu
            .paint_target
            .as_ref()
            .expect("liquify_compiled::flush_dabs requires paint_target");
        let canvas_ext = paint_target.canvas_extent();
        ...
        let outputs = gpu
            .slot_outputs_owned
            .as_ref()
            .expect("liquify_compiled::flush_dabs requires slot_outputs_owned");
    ...
    fn compile_wgsl(&self, cctx: &CompileWgslCtx) -> Result<NodeWgsl, String> {
        ...
        // the disc, matching the dispatch-path liquify.
        ...
        // Fragment body: `local_dist` and `canvas_pos` come from the
        // framework wrapper; the framework already discards past
        // `d.bbox_radius`. We additionally discard past `local_dist >=
        // 1.0` so the warp stays inside the disc (the framework's
        // discard kicks in for `bbox_radius < radius` cases too, but
        // when extent contribution lands at 1.0 the two conditions
        // coincide).
        ...
        wgsl.body = format!(
            "    if (local_dist >= 1.0) {{ discard; }}\n\
             ...
             \x20   let source_pos = canvas_pos - dir * displacement * f;\n\
             ...
             \x20   let original_uv = (canvas_pos  - d.{copy_origin_field}) / mirror_dims;\n\
